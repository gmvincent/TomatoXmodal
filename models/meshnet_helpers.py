""" MeshNet2 Structural + Spatial Descriptors & Layers """
import torch
...
class SpectralDescriptor(torch.nn.Module):
    """
...
class NormalDescriptor(torch.nn.Module):
    """
    ...
    def __init__(self, num_kernel=64):
        ...
        self.spatial_mlp = torch.nn.Sequential(
            torch.nn.Conv1d(3, self.num_kernel, 1),
            torch.nn.BatchNorm1d(self.num_kernel),
            torch.nn.ReLU(),
        )
...
class NeighborResampleUniform(torch.nn.Module):
    r"""
...
class NeighborResampleWeighted(torch.nn.Module):
    r"""
...
class AlphaBetaGamma(torch.nn.Module):
    """
...
class ConvSurface(torch.nn.Module):
    """
    ...
    """
    def __init__(self, num_faces=500, num_neighbor=3, num_samples_per_neighbor=4, rs_mode="Weighted", num_kernel=64):
        """
        Args:
            num_faces: number of faces in each mesh
        ...
        self.num_faces = num_faces
        self.num_samples_per_neighbor = num_samples_per_neighbor
        ...
        self.directions = torch.nn.Parameter(torch.FloatTensor(3, self.num_kernel))
        self.relu = torch.nn.ReLU(inplace=True)
        self.bn = torch.nn.BatchNorm1d(self.num_kernel)
        self.initialize()
...
class PointDescriptor(torch.nn.Module):
    """
...
class MaxPoolFaceFeature(torch.nn.Module):
    r"""
...
class ConvFace(torch.nn.Module):
    r"""
...
class ConvFaceBlock(torch.nn.Module):
    """
...
class MeshBlock(torch.nn.ModuleDict):
    """
