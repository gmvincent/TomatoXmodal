...
def main():
    root_dir = '/home/gmvincen/cmwilli5_drive/gmvincen_data/tomato_diseases/cleaned_phenospex_polygons'
    out_dir = '/home/gmvincen/cmwilli5_drive/gmvincen_data/tomato_diseases/voxel_data'

    all_files = glob.glob(os.path.join(root_dir, "**", "*.ply"), recursive=True)
    mesh_files = [f for f in all_files if 'trian' not in os.path.basename(f)]
    
    for fp in tqdm(mesh_files, desc="Voxelizing Files", total=len(mesh_files), unit="file"):
        if fp.endswith('.ply') and 'X' in fp:
            try:
                voxel, scalars = process_ply(fp)

                voxel_out, scalar_out = rename_files(os.path.basename(fp))
                voxel_path = os.path.join(out_dir, voxel_out)
